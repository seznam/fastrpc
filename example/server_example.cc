...
// this example contains two server methods first is in the class and second is global
...
public:
    ...
    {
        //automatic integrity check of input parameters(int,string) 
...
class Callbacks_t:public MethodRegistry_t::Callbacks_t
{
                             ...
                             const MethodRegistry_t::TimeDiff_t &time)
    {
        ...
        printf("Method: %s return %s. Execution time %ld sec and %ld microsec\n",
               methodName.c_str(),str.c_str(),time.second, time.usecond);
...
int main(int argc, char *argv[])
{
    ...
    server.registry().registerMethod("test1",
                                     boundMethod(&Test_t::methodTest,test),"S:is",
                                     "Method in the class");
    server.registry().registerMethod("test2",
                                     unboundMethod(methodTest,a),"S:is",
                                     "Global method");
    //register head method FOR HTTP HEAD (for content switch)
